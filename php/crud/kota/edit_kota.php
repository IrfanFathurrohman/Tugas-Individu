<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Document</title>
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.2/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-T3c6CoIi6uLrA9TneNEoa7RxnatzjcDSCmG1MXxSR1GAsXEV/Dwwykc2MPK8M2HN" crossorigin="anonymous">

</head>
<body style="background-color: rgb(77, 77, 77);">
    <div class="container" style="color: white;">
        <div class="row">
            <div class="col-md-10 mx-auto" style="background-color: rgb(0, 0, 0);">
            <h3 class="my-5" align="center">Form Tambah Data Kota</h3>

            <?php
            include "../koneksi.php";

            $id_kota = $_GET['id_kota'];

            echo $id_kota;

            $kota = mysqli_query($conn,"SELECT * FROM kota WHERE id_kota='$id_kota'");
            while($data = mysqli_fetch_array($kota)){
                $nama_kota = $data['nama_kota'];
                $ongkir = $data['ongkos_kirim'];
            } ?>
                <form action="proses_edit_kota.php?id_kota=<?php echo $id_kota; ?>" method="post" style="margin: 100px;">
                    <div class="row mb-3">
                        <label class="col-sm-2 col-form-label">Nama Kota</label>
                        <div class="col-sm-8">
                            <input type="text" class="form-control" name="nama_kota" value="<?php echo $nama_kota; ?>">
                        </div>
                    </div>
                    <div class="row mb-3">
                        <label class="col-sm-2 col-form-label">Ongkos Kirim</label>
                        <div class="col-sm-8">
                            <input type="number" class="form-control" name="ongkos_kirim" value="<?php echo $ongkir; ?>">
                        </div>
                    </div>
                    
                    
                    <button type="submit" name="submit" class="btn btn-primary" style="margin-left: 150px; margin-top: 20px; margin-bottom: 140px;">Simpan</button>
                </form>
            </div>
        </div>
    </div>
</body>
</html>